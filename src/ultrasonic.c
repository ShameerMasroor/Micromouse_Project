...
static ultrasonic ultrasonic = {
    .trig_spec = GPIO_DT_SPEC_GET(ULTRASONIC_TRIG, gpios),
    .echo_spec = GPIO_DT_SPEC_GET(ULTRASONIC_ECHO, gpios),
};

void init_ultrasonic(ultrasonic *sensor) {
    int ret;
...
uint32_t measure_distance(ultrasonic *sensor) {
